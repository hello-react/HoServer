...
const defTableColumns = Common.setDefaultColumn([
    {
        title: "接口分类",
        dataIndex: "main_catname",
        align: 'center',
        ellipsis: false
    },
    {
        title: "子分类",
        dataIndex: "second_catname",
        align: 'center',
        ellipsis: false
    },
    {
...
class ApiManage extends TableLayout {
    constructor() {
        ...
        defTableColumns[0].render = (value, row) => {
            const obj = {
                children: (row.main_cat === '_default') ? (value || '默认分类') : (
                    <span onClick={() => this.handleChangeCatName(row.main_cat, value)}>
                        {value}{`${row.type == 1 ? ' (内置)' : (row.type == 2 ? ' (插件)' : '')}`}
                        <br />
                        <Tooltip title="可以通过修改系统字典中的 sys_category 字典相关键值修改分类名称">
                            {row.main_cat}
                        </Tooltip>
                    </span>
                ),
        ...
        defTableColumns[1].render = (value, row) => {
            const obj = {
                children: (row.second_cat === '_default') ? (value || '默认分类') : (
                    <span onClick={() => this.handleChangeCatName(row.second_cat, value)}>
                        {value || '默认分类'}{' '}
                        <br />
                        <Tooltip title="可以通过修改系统字典中的 sys_category 字典相关键值修改分类名称">
                            {row.second_cat}
    ...
    loadData = async params => {
        ...
        for (let i=0; i<data.length; i++) {
            const row = data[i]
            const mainCat = row.main_cat
            const secondCat = row.second_cat
