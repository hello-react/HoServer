...
const before = async context => {
    ...
    if (api.input_example === undefined) {
...
const after = async context => {
    ...
    if (!context.error && context.result) {
        if (api.output_example === undefined || !(api.out_fields && api.out_fields.length > 0)) {
...
function fillObjectFields(fields, key, obj) {
    ...
    if (resultType !== 'object') {
        if (!key) {
            fields.push({ name: 'result', type: resultType === 'string' ? 'char' : resultType, description: '操作结果' })
        } else {
        ...
        if (obj instanceof Array) {
            if (obj.length > 0) {
                ...
                if (key) {
                    fields.push({ name: key, type: 'array-of-' + (arrayType === 'string' ? 'char' : arrayType), description: '' })
                }
