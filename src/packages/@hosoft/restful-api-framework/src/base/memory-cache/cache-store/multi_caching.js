...
const CallbackFiller = require('./callback_filler')
const utils = require('./utils')
const isObject = utils.isObject
const parseWrapArguments = utils.parseWrapArguments
 ...
 */
const multiCaching = function (caches, options) {
    const self = {}
    options = options || {}

    const Promise = options.promiseDependency || global.Promise
    ...
    function getFromHighestPriorityCachePromise() {
        const args = Array.prototype.slice.apply(arguments).filter(function (v) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            args.push(cb)
            getFromHighestPriorityCache(...args)
        })
    }
        ...
        async.eachSeries(
            ...
            function (cache, next) {
                ...
                if (multi) {
                    ...
                    const _args = Array.prototype.slice.apply(keysToFetch)
                    _args.push(options)
                    _args.push(callback)
    ...
    function setInMultipleCachesPromise() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            args.push(cb)
            setInMultipleCaches(...args)
        })
    }

    function setInMultipleCaches() {
        const args = Array.prototype.slice.apply(arguments)
        const _caches = Array.isArray(args[0]) ? args.shift() : caches
        ...
        async.each(
            ...
            function (cache, next) {
                const _isCacheableValue = getIsCacheableValueFunction(cache)
                const keysValues = Array.prototype.slice.apply(args)
     ...
     */
    self.wrap = function () {
        const parsedArgs = parseWrapArguments(Array.prototype.slice.apply(arguments))
        const keys = parsedArgs.keys
        const work = parsedArgs.work
        const options = parsedArgs.options
        const cb = parsedArgs.cb
    ...
    function wrapMultiple(keys, work, options, cb) {
         ...
         */
        const combinedKey = keys.reduce(function (acc, k) {
            return acc + k
        }, '')

        const hasKey = callbackFiller.has(combinedKey)
