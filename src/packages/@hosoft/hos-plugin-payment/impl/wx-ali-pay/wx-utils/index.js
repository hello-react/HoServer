...
/**
 * Get the client token from the WeChat server through appId and appSecret
 * @param callback
 */
    ...
    return new Promise((resolve, reject) => {
        request.get(
            ...
            (err, res, body) => {
                ...
                // console.log(body);
                if (body.errcode) {
                    return reject(`get access token failed[${body.errcode}], ${body.errmsg}`)
                }
    ...
    return new Promise((resolve, reject) => {
        request(options, async (tokenErr, tokenRes, tokenBody) => {
            if (tokenErr || !tokenRes) {
                return reject(tp('wxAuthFail', { message: JSON.stringify(tokenErr) }))
            }
            ...
            if (tokenBody.errcode / 1 > 0) {
                return reject('get user openid failed: ' + JSON.stringify(tokenBody))
            }
            ...
            request(options, async (userErr, userRes, userBody) => {
                if (userErr && userRes) {
                    return reject('get wechat user information failed: ' + JSON.stringify(userErr))
                }
...
async function doPrepay(
    ...
    formData +=
        '<sign>' +
        pay.paysignjsapi(app_id, attach, body, mch_id, nonce_str, notify_url, openid, tid, device_ip, total_fee, trade_type, api_key) +
        '</sign>'
    formData += '</xml>'
    ...
    if (pResObj.xml.return_code[0] === 'FAIL') {
        ...
        if (pResObj.xml.result_code[0] === 'FAIL') {
            throw pResObj.xml.err_code_des[0]
        }
